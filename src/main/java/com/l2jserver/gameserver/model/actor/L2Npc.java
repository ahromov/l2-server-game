...
public class L2Npc extends L2Character {
	...
	/**
	 * Send a packet SocialAction to all L2PcInstance in the _KnownPlayers of the
	 * L2NpcInstance and create a new RandomAnimation Task.
	 * 
	 * @param animationId
	 */
	...
	/**
	 * Send a packet NpcInfo with state of abnormal effect to all L2PcInstance in
	 * the _KnownPlayers of the L2NpcInstance.
	 */
	@Override
	public void updateAbnormalEffect() {
		// Send a Server->Client packet NpcInfo with state of abnormal effect to all
		// L2PcInstance in the _KnownPlayers of the L2NpcInstance
	 ...
	 * @return {@code true} if the player is clan leader and owner of a castle of
	 *         fort that this NPC belongs to, {@code false} otherwise
	 */
	...
	/**
	 * Open a quest or chat window on client with the text of the L2NpcInstance in
	...
	/**
	 * Send a Server->Client packet NpcHtmlMessage to the L2PcInstance in order to
	 * display the message of the L2NpcInstance.
	 * 
	 ...
	 * <li>if the file doesn't exist on the server : <B>data/html/npcdefault.htm</B>
	 * (message : "I have nothing to say to you")</li>
	 * </ul>
	 * 
	 * @param npcId The Identifier of the L2NpcInstance whose text must be display
	 * @param val   The number of the page to display
	 * @return the pathfile of the selected HTML file in function of the npcId and
	 *         of the page number.
	 */
	 ...
	 * <B><U>Actions</U>:</B>
	 * <ul>
	 * <li>Get the text of the selected HTML file in function of the npcId and of
	 * the page number</li>
	 * <li>Send a Server->Client NpcHtmlMessage containing the text of the
	 * L2NpcInstance to the L2PcInstance</li>
	 * <li>Send a Server->Client ActionFailed to the L2PcInstance in order to avoid
	 * that the client wait another packet</li>
	 * </ul>
	 * 
	...
	public void showChatWindow(L2PcInstance player, int val) {
		...
		case 36402:
			if (player.getOlympiadBuffCount() > 0) {
				filename = (player.getOlympiadBuffCount() == Config.ALT_OLY_MAX_BUFFS
						? Olympiad.OLYMPIAD_HTML_PATH + "olympiad_buffs.htm"
		...
		default:
			...
			// Get the text of the selected HTML file in function of the npcId and of the
			// page number
			filename = (getHtmlPath(npcId, val));
			break;
		}

		// Send a Server->Client NpcHtmlMessage containing the text of the L2NpcInstance
		// to the L2PcInstance
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
	...
	/**
	 * Open a chat window on client with the text specified by the given file name
	 * and path, relative to the datapack root.
	 * 
	 * @param player   The L2PcInstance that talk with the L2NpcInstance
	 ...
	 * other L2PcInstance to inform</li>
	...
	/**
	 * Remove the L2NpcInstance from the world and update its spawn object (for a
	 * complete removal use the deleteMe method).<br>
	 * <B><U>Actions</U>:</B>
	 * <ul>
	...
	public boolean deleteMe() {
		...
		// then cancel Attack or Cast and notify AI
		try {
	...
	// This is only useful for a few NPCs and is most likely going to be called from
	// AI
	...
	public void showNoTeachHtml(L2PcInstance player) {
		...
		if (this instanceof L2WarehouseInstance) {
			...
			if (html == null) {
				html = HtmCache.getInstance().getHtm(player.getHtmlPrefix(),
						"data/scripts/ai/npc/Trainers/HealerTrainer/" + npcId + "-noteach.html");
			}
		}
		...
		if (html == null) {
			...
			noTeachMsg.setHtml(
					"<html><body>I cannot teach you any skills.<br>You must find your current class teachers.</body></html>");
		} else {
	...
	protected final void notifyQuestEventSkillFinished(Skill skill, L2Object target) {
		if ((target != null) && target.isPlayable()) {
			EventDispatcher.getInstance()
					.notifyEventAsync(new OnNpcSkillFinished(this, target.getActingPlayer(), skill), this);
		}
	}
	 ...
	 * 
	 * @param val value to store
	 * @return {@code true} if stored script value equals given value, {@code false}
	 *         otherwise
	 */
	...
	public boolean hasAIValue(final String paramName) {
		return (getSpawn() != null) && (getSpawn().getName() != null)
				&& NpcPersonalAIData.getInstance().hasAIValue(getSpawn().getName(), paramName);
	}
	...
	public boolean isInMySpawnGroup(L2Npc npc) {
		return ((getSpawn() != null) && (npc.getSpawn() != null) && (getSpawn().getName() != null)
				&& (getSpawn().getName().equals(npc.getSpawn().getName())));
	}
	...
	/**
	 * @return {@code true} if {@link NpcVariables} instance is attached to current
	 *         player's scripts, {@code false} otherwise.
	 */
	...
	public void broadcastEvent(String eventName, int radius, L2Object reference) {
		for (L2Object obj : L2World.getInstance().getVisibleObjects(this, radius)) {
			if (obj.isNpc() && obj.hasListener(EventType.ON_NPC_EVENT_RECEIVED)) {
				EventDispatcher.getInstance()
						.notifyEventAsync(new OnNpcEventReceived(eventName, this, (L2Npc) obj, reference), obj);
			}
		}
	}
	...
	public void sendScriptEvent(String eventName, L2Object receiver, L2Object reference) {
		EventDispatcher.getInstance()
				.notifyEventAsync(new OnNpcEventReceived(eventName, this, (L2Npc) receiver, reference), receiver);
	}
	...
	public Location getPointInRange(int radiusMin, int radiusMax) {
		...
		return new Location((int) (getX() + (radius * Math.cos(angle))), (int) (getY() + (radius * Math.sin(angle))),
				getZ());
	}
	...
	public L2ItemInstance dropItem(L2PcInstance player, int itemId, long itemCount) {
		...
		for (int i = 0; i < itemCount; i++) {
			...
			if (!Config.LIST_PROTECTED_ITEMS.contains(itemId)) {
				if (((Config.AUTODESTROY_ITEM_AFTER > 0) && !item.getItem().hasExImmediateEffect())
						|| ((Config.HERB_AUTO_DESTROY_TIME > 0) && item.getItem().hasExImmediateEffect())) {
	...
	public boolean isVisibleFor(L2PcInstance player) {
		if (hasListener(EventType.ON_NPC_CAN_BE_SEEN)) {
			final TerminateReturn term = EventDispatcher.getInstance().notifyEvent(new OnNpcCanBeSeen(this, player),
					this, TerminateReturn.class);
			if (term != null) {
